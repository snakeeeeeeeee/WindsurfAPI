...
- Investigated the later CCTest report that still showed 99% hit rate. Dashboard API revealed stale `WINDSURFAPI_ANTHROPIC_REPORTED_CACHE_HIT_RATE=0.85`, and CCTest's top-level hit rate appears to use a read-only denominator (`cache_read / (input + cache_read)`) rather than including `cache_creation`.
- Added `WINDSURFAPI_ANTHROPIC_REPORTED_CACHE_TARGET_INPUT_FLOOR` plus Dashboard/runtime-config/i18n support. When enabled with target `0.90`, reported fresh input is floored to `ceil(cache_read * 0.1 / 0.9)`, making read-only displays land near 90%.
- Verification passed: `node --check src/handlers/messages.js`, `node --check src/runtime-config.js`, `node --test test/messages.test.js`, `node --test test/runtime-config-sqlite.test.js test/dashboard-syntax.test.js`, and `git diff --check`.
- Hot-copied the new files into local Docker, restarted `windsurfapi-windsurf-api-1`, cleared the stale cache-hit override, enabled the new input floor, and verified in-container usage math: `input_tokens=7013`, `cache_read_input_tokens=63113`, `cache_creation_input_tokens=8`, read-only hit rate `0.9`.
- Added `CASCADE_DISABLE_HISTORY_TRIM` to let operators explicitly disable Cascade history trimming. The switch makes `cascadeHistoryBudget()` return `Infinity`, so fresh sends and resume-failure rebuilds keep the full caller history instead of enforcing the 400KB/900KB budgets.
- Exposed the no-trim switch in Dashboard runtime env and SQLite-backed runtime config, added a regression test in `test/client-content.test.js`, and verified with `node --check src/client.js`, `node --check src/runtime-config.js`, `node --test test/client-content.test.js`, `node --test test/runtime-config-sqlite.test.js test/dashboard-syntax.test.js`, and `git diff --check`.
- Hot-copied the no-trim changes into local Docker, restarted `windsurfapi-windsurf-api-1`, and enabled `CASCADE_DISABLE_HISTORY_TRIM=1` through the Dashboard env API.
- Fixed Claude CLI `/compact` compatibility without re-enabling history trimming: `/v1/messages` now detects conversation-compaction/handoff-summary requests, removes this turn's `tools/tool_choice`, and passes `__forceTextResponse` into chat handling.
- In force-text mode, chat handling preserves historical tool results as context but suppresses native bridge, tool preamble injection, and stream/non-stream tool-call parsing, so tool-shaped summary text cannot become `tool_use`.
- Verification passed: `node --check src/handlers/messages.js`, `node --check src/handlers/chat.js`, `node --test test/messages.test.js test/tool-emulation-end-to-end.test.js`, `node --test test/tool-emulation.test.js test/tool-emulation-allowlist.test.js test/tool-emulation-salvage.test.js test/chat-reuse.test.js test/stream-cache-policy.test.js test/stream-error.test.js test/stream-stall.test.js`, and `git diff --check`.
- Hot-copied `src/handlers/chat.js` and `src/handlers/messages.js` into `windsurfapi-windsurf-api-1`, restarted the container, waited for Docker health to return `healthy`, and confirmed `CASCADE_DISABLE_HISTORY_TRIM=1` remained active.
